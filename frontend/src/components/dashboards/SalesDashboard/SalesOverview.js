...
const SalesDashboard = () => {
  ...
  const fetchSalesData = async () => {
    try {
      const response = await axios.get("https://iwb-2213.vercel.app/api/sales/overview", {
  ...
  const handleAddProduct = async () => {
    try {
      setLoading(true);
      await axios.post("https://iwb-2213.vercel.app/api/products", newProduct, {
        headers: { Authorization: `Bearer ${token}` },
      });
      alert("Product added!");
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    try {
      await axios.delete(`https://iwb-2213.vercel.app/api/products/${productId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
